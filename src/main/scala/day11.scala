...
case class Day11()
{
  ...
  def nextPassword(password: String) : String = {
    val (_, pwd) = password.foldRight((1, "")) {
      case (c, (offset, pwd)) if (c == 'i') => (0, 'j'+ "a"*pwd.length)
      case (c, (offset, pwd)) if (c == 'o') => (0, 'p'+ "a"*pwd.length)
      case (c, (offset, pwd)) if (c == 'l') => (0, 'm'+ "a"*pwd.length)
